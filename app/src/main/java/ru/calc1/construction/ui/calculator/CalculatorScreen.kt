...
import ru.calc1.construction.R
		...
		if (currentCalculator == null) {
			...
			Column(
				...
				if (results.isNotEmpty()) {
					...
					val calculatorCategory = remember(currentCalculator.categoryId) {
						CalculatorRepository.getCategories().firstOrNull { it.id == currentCalculator.categoryId }
					}
					val showCalculationDetails = calculatorCategory?.accessLevel == AccessLevel.FREE
 ...
 */
@Composable
private fun UsageExampleCard(example: ru.calc1.construction.domain.model.UsageExample) {
	Card(
		modifier = Modifier.fillMaxWidth(),
		colors = CardDefaults.cardColors(
			containerColor = MaterialTheme.colorScheme.surfaceVariant
